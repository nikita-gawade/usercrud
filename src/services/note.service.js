...
const deleteAllNotes = async ({ account_id }) => {
  ...
  return getList({
    account_id, page_no: 1, page_size: 100, sort_by: 'created_at', sort_order: 'desc',
  });
};

const deleteNotes = async ({ account_id, ids }) => NoteModel.destroy({
  where: {
    account_id,
    id: ids,
  },
...
const updateNotes = async ({ account_id, notes }) => {
  notes.forEach((note) => {
    // eslint-disable-next-line no-param-reassign
    note.account_id = account_id;
  });
  await NoteModel.bulkCreate(notes, {
